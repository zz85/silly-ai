    ...
    pub fn summary(&self) -> String {
        ...
        if !self.transcription.is_empty() {
            ...
            out.push_str(&format!(
                ...
                self.transcription.len(), avg.as_millis(), min.as_millis(), max.as_millis(), rtf
            ));
        }
        ...
        if !self.tts.is_empty() {
            ...
            out.push_str(&format!(
                ...
                self.tts.len(), avg.as_millis(), min.as_millis(), max.as_millis(), rtf
            ));
        }
        
        if !self.llm.is_empty() {
            let avg_ttft: Duration = self.llm.iter().map(|s| s.ttft).sum::<Duration>() / self.llm.len() as u32;
            let total_time: Duration = self.llm.iter().map(|s| s.total).sum();
            let total_tokens: usize = self.llm.iter().map(|s| s.tokens).sum();
            let tps = if total_time.as_secs_f64() > 0.0 { total_tokens as f64 / total_time.as_secs_f64() } else { 0.0 };
            out.push_str(&format!(
                "LLM (n={}): TTFT={:.0}ms avg, {:.1} tok/s ({} tokens)\n",
                self.llm.len(), avg_ttft.as_millis(), tps, total_tokens
            ));
        }
...
impl<'a> Timer<'a> {
    pub fn new(stats: &'a SharedStats, kind: StatKind, input_size: usize) -> Self {
        Self { start: Instant::now(), stats, kind, input_size }
    }
...
impl LlmTimer {
    pub fn new(stats: SharedStats) -> Self {
        Self { start: Instant::now(), first_token: None, stats }
    }
